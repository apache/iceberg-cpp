...
TEST(StatusTest, AndStatus) {
  Status a = Status::OK();
  Status b = Status::OK();
  Status c = Status::IOError("file error");
  Status d = Status::NotImplemented("not implement value");
  ...
  res = d & c;
  ASSERT_TRUE(res.IsNotImplemented());
  ...
  // With rvalues
  res = Status::OK() & Status::IOError("foo");
  ASSERT_TRUE(res.IsIOError());
  res = Status::IOError("foo") & Status::OK();
  ASSERT_TRUE(res.IsIOError());
  res = Status::IOError("foo") & Status::NotImplemented("bar");
  ASSERT_TRUE(res.IsIOError());

  res = Status::OK();
  res &= Status::OK();
  ASSERT_TRUE(res.ok());
  res &= Status::IOError("foo");
  ASSERT_TRUE(res.IsIOError());
  res &= Status::UnknownError("bar");
  ASSERT_TRUE(res.IsIOError());
}

TEST(StatusTest, TestEquality) {
  ASSERT_EQ(Status(), Status::OK());
  ASSERT_EQ(Status::IOError("error"), Status::IOError("error"));
  ASSERT_NE(Status::IOError("error"), Status::OK());
  ASSERT_NE(Status::IOError("error"), Status::IOError("other error"));
}
