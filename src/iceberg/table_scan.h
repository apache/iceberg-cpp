...
/// \brief A scan task for deletes generated by adding deletion vectors to the table.
///
/// Suppose snapshot S1 contains data files F1 and F2. Then snapshot S2 adds a deletion
/// vector, DV1, that deletes records from F2, and snapshot S3 replaces DV1 with DV2 that
/// deletes further records from F2. A scan for changes from S2 to S3 (inclusive) should
/// include the following tasks:
/// - DeletedRowsScanTask(file=F2, added-deletes=[DV1], existing-deletes=[], snapshot=S2)
/// - DeletedRowsScanTask(file=F2, added-deletes=[DV2], existing-deletes=[DV1],
///   snapshot=S3)
///
/// Readers consuming these tasks should produce the records that are deleted by the added
/// deletes but not by the existing deletes, with metadata like change ordinal and commit
/// snapshot ID.
class ICEBERG_EXPORT DeletedRowsScanTask : public ChangelogScanTask {
 public:
  /// \brief Construct a DeletedRowsScanTask.
  ///
  /// \param change_ordinal Position in the changelog order (0-based).
  /// \param commit_snapshot_id The snapshot ID that committed the added deletes.
  /// \param data_file The data file containing the deleted rows.
  /// \param added_deletes Delete files committed by the snapshot for this data file.
  /// \param existing_deletes Delete files that applied to this data file before the
  /// snapshot.
  /// \param residual_filter Optional residual filter to apply after reading.
  DeletedRowsScanTask(int32_t change_ordinal, int64_t commit_snapshot_id,
                      std::shared_ptr<DataFile> data_file,
  ...
  /// \brief The data file containing the deleted rows.
  const std::shared_ptr<DataFile>& data_file() const { return data_file_; }

  /// \brief Delete files added by the commit snapshot that apply to the data file.
  ///
  /// Records removed by these delete files appear as deletes in the changelog.
  const std::vector<std::shared_ptr<DataFile>>& added_deletes() const {
    return delete_files_;
  }

  /// \brief Delete files that existed before the commit snapshot and must be applied
  /// prior to determining which records are deleted by added_deletes().
  ///
  /// Records removed by these delete files do not appear in the changelog.
  const std::vector<std::shared_ptr<DataFile>>& existing_deletes() const {
    return existing_deletes_;
  }

 private:
  std::vector<std::shared_ptr<DataFile>> existing_deletes_;
};
...
///
/// Delete files are supported only for format version 3 tables, where row-level deletes
