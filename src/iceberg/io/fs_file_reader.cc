...
int64_t FsFileReader::read(ReadRange range, void* buffer) {
  ...
  if (range.offset < 0 || range.offset + range.length > file_size_) {
    throw IcebergError(std::format("Invalid read range: [{}, {})", range.offset,
  ...
  if (input_file_.fail()) {
    throw IcebergError(std::format("Failed to seek to offset: {}", range.offset));
  }
...
std::future<int64_t> FsFileReader::readAsync(ReadRange range, void* buffer) {
  return std::async(std::launch::async, [this, range, buffer]() -> int64_t {
    return this->read(range, buffer);
  });
}
