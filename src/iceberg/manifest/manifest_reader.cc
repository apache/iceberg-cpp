...
Status ParseIntegerField(const ArrowArrayView* array_view, Container& container,
  ...
  using T = decltype([] {
    using MayBeOptional = std::decay_t<
        std::invoke_result_t<Accessor&, std::ranges::range_reference_t<Container>>>;
    if constexpr (requires { typename MayBeOptional::value_type; }) {
      return std::type_identity<typename MayBeOptional::value_type>();
    } else {
      return std::type_identity<MayBeOptional>();
    }
  }())::type;
  return ParseField(
...
void ParseIntegerVectorField(const ArrowArrayView* view, int64_t length,
                             Container& container, Accessor accessor) {
  using T = decltype([] {
    using MayBeOptional = std::ranges::range_value_t<std::decay_t<
        std::invoke_result_t<Accessor&, std::ranges::range_reference_t<Container>>>>;
    if constexpr (requires { typename MayBeOptional::value_type; }) {
      return std::type_identity<typename MayBeOptional::value_type>();
    } else {
