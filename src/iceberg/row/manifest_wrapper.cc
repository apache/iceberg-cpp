...
Result<ViewValue> PartitionFieldSummaryStructLike::GetField(size_t pos) const {
  if (pos >= num_fields()) {
  ...
  switch (pos) {
    ...
    case 2: {
      if (summary_.get().lower_bound.has_value()) {
        return summary_.get().lower_bound.value();
      }
      return {};
    }
    case 3: {
      if (summary_.get().upper_bound.has_value()) {
        return summary_.get().upper_bound.value();
      }
      return {};
    }
    default:
...
Result<ViewValue> PartitionFieldSummaryArrayLike::GetElement(size_t pos) const {
  if (pos >= size()) {
    return InvalidArgument("Invalid partition field summary index: {}", pos);
  }
  if (summary_ == nullptr) {
...
Result<ViewValue> ManifestFileStructLike::GetField(size_t pos) const {
  if (pos >= num_fields()) {
  ...
  switch (field) {
    ...
    case ManifestFileField::kKeyMetadata:
      return manifest_file.key_metadata;
