...
class ICEBERG_REST_EXPORT HttpClient {
 public:
  ...
  static Result<std::unique_ptr<HttpClient>> Make(RestCatalogConfig config);
 ...
 private:
  ...
  HttpClient(std::shared_ptr<const RestCatalogConfig> config,
  ...
  std::shared_ptr<const RestCatalogConfig> config;
  cpr::Url base_url;
