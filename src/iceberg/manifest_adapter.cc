...
std::unique_ptr<ManifestFile> ManifestEntryAdapter::ToManifestFile() const {
  auto manifest_file = std::make_unique<ManifestFile>();
  manifest_file->partition_spec_id = partition_spec_->spec_id();
  manifest_file->content = content_;
  // sequence_number and min_sequence_number with UNASSIGNED_SEQUENCE_NUMBER will be
