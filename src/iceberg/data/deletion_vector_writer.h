...
struct ICEBERG_DATA_EXPORT DeleteWriteResult {
  ...
  /// Previously written, file-scoped delete files that were merged and should be
...
struct ICEBERG_DATA_EXPORT PreviousDeletes {
  /// The previously deleted positions, or null if none.
  std::shared_ptr<PositionDeleteIndex> index;
  /// The delete files the positions came from; file-scoped ones are reported as
  /// rewritten.
  std::vector<std::shared_ptr<DataFile>> delete_files;
};
...
struct ICEBERG_DATA_EXPORT DeletionVectorWriterOptions {
  ...
  /// Optional hook to load previously written deletes for a data file so they
  /// can be merged into the new deletion vector, with the old delete files
  /// reported as rewritten.
  std::function<Result<PreviousDeletes>(std::string_view)> load_previous_deletes;
};
