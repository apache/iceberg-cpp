...
class ICEBERG_EXPORT RewriteManifests : public SnapshotUpdate {
 ...
 private:
  ...
  Status ValidateAddedManifests() const;
  Status ValidateActiveFiles() const;
  Result<std::vector<ManifestEntry>> Entries(const ManifestFile& manifest) const;
  Result<std::vector<DataFile>> ActiveFiles(
      const std::vector<ManifestFile>& manifests) const;
  Result<ManifestFile> FillMissingCounts(const ManifestFile& manifest) const;
