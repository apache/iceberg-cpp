...
class ICEBERG_EXPORT UnboundPredicate : public Predicate<UnboundTerm<B>>,
 ...
 public:
  ...
  std::shared_ptr<NamedReference> reference() override {
    return Base::term_->reference();
  }

  std::string ToString() const override;

  /// \brief Bind this UnboundPredicate.
  Result<std::unique_ptr<Expression>> Bind(const Schema& schema,
...
class ICEBERG_EXPORT BoundPredicate : public Predicate<BoundTerm>, public Bound {
 public:
  ...
  ~BoundPredicate() override = default;
  ...
  /// \brief Returns a string representation of this predicate.
  std::string ToString() const override;

  /// \brief Returns whether this is a unary predicate (tests for null, not-null, etc.).
  virtual bool is_unary_predicate() const { return false; }

  /// \brief Returns whether this is a literal predicate (compares against a literal).
  virtual bool is_literal_predicate() const { return false; }

  /// \brief Returns whether this is a set predicate (tests membership in a set).
  virtual bool is_set_predicate() const { return false; }
};
...
class ICEBERG_EXPORT BoundUnaryPredicate : public BoundPredicate {
 public:
  ...
  ~BoundUnaryPredicate() override = default;

  Result<bool> Test(const Literal::Value& value) const override;

  bool is_unary_predicate() const final { return true; }
};
...
class ICEBERG_EXPORT BoundLiteralPredicate : public BoundPredicate {
 public:
  ...
  ~BoundLiteralPredicate() override = default;
  ...
  bool is_literal_predicate() const final { return true; }
...
class ICEBERG_EXPORT BoundSetPredicate : public BoundPredicate {
 public:
  ...
  ~BoundSetPredicate() override = default;
  ...
  bool is_set_predicate() const final { return true; }
