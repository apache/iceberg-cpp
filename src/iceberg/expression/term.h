...
class ICEBERG_EXPORT BoundReference
 ...
 public:
  ...
  const std::shared_ptr<Type>& type() const override { return field_->type(); }
 ...
 private:
  std::shared_ptr<SchemaField> field_;
};
...
class ICEBERG_EXPORT UnboundTransform : public UnboundTerm<class BoundTransform> {
 public:
  ...
  ~UnboundTransform() override = default;

  std::string ToString() const override;

  Result<std::unique_ptr<BoundTransform>> Bind(const Schema& schema,
                                               bool case_sensitive) const override;
...
class ICEBERG_EXPORT BoundTransform : public BoundTerm {
 public:
  ...
  ~BoundTransform() override = default;
  ...
  const std::shared_ptr<Type>& type() const override { return ref_->type(); }

  bool MayProduceNull() const override { return ref_->MayProduceNull(); }
