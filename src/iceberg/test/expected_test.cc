...
TEST(ExpectedTest, UnexpectedEquality) {
  const iceberg::unexpected<std::string> error1("error1");
  const iceberg::unexpected<std::string> error1_copy("error1");
  const iceberg::unexpected<std::string> error2("error2");

  EXPECT_EQ(error1, error1_copy);
