...
TEST_P(BlobMetadataJsonTest, RoundTrip) {
  ...
  EXPECT_THAT(BlobMetadataFromJson(expected), HasValue(::testing::Eq(param.blob)));
}
...
TEST(PuffinJsonTest, FileMetadataRoundTrip) {
  ...
  EXPECT_THAT(FileMetadataFromJson(expected_json), HasValue(::testing::Eq(metadata)));
}
...
TEST(PuffinJsonTest, FileMetadataStringRoundTrip) {
  ...
  EXPECT_THAT(FileMetadataFromJsonString(json_str), HasValue(::testing::Eq(metadata)));
}
