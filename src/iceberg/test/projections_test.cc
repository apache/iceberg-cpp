...
TEST_F(ProjectionsTest, IdentityProjectionInclusive) {
  ...
  for (const auto& predicate : predicates) {
    ...
    auto evaluator = Projections::Inclusive(*spec, true);
...
TEST_F(ProjectionsTest, IdentityProjectionStrict) {
  ...
  for (const auto& predicate : predicates) {
    ...
    // Project the bound predicate
    auto evaluator = Projections::Strict(*spec, true);
    ICEBERG_UNWRAP_OR_FAIL(auto projected_expr, evaluator->Project(bound_pred));
...
TEST_F(ProjectionsTest, CaseInsensitiveIdentityProjection) {
  ...
  for (const auto& predicate : predicates) {
    ...
    auto evaluator = Projections::Inclusive(*spec, false);
    ICEBERG_UNWRAP_OR_FAIL(auto projected_expr, evaluator->Project(bound_pred));
...
class BucketingProjectionTest : public ::testing::Test {
 protected:
  void AssertProjectionStrict(const PartitionSpec& spec,
  ...
  void AssertProjectionStrictValue(const PartitionSpec& spec,
                                   const std::shared_ptr<Expression>& filter,
                                   Expression::Operation expected_op) {
    auto evaluator = Projections::Strict(spec, true);
    ICEBERG_UNWRAP_OR_FAIL(auto projection, evaluator->Project(filter));
  ...
  void AssertProjectionInclusive(const PartitionSpec& spec,
                                 const std::shared_ptr<Expression>& filter,
                                 Expression::Operation expected_op,
                                 const std::string& expected_literal) {
    auto evaluator = Projections::Inclusive(spec, true);
  ...
  void AssertProjectionInclusiveValue(const PartitionSpec& spec,
                                      const std::shared_ptr<Expression>& filter,
                                      Expression::Operation expected_op) {
    auto evaluator = Projections::Inclusive(spec, true);
    ICEBERG_UNWRAP_OR_FAIL(auto projection, evaluator->Project(filter));
...
TEST_F(BucketingProjectionTest, BucketIntegerStrict) {
  ...
  AssertProjectionStrict(*spec, bound_not_equal, Expression::Operation::kNotEq, "6");
  AssertProjectionStrictValue(*spec, bound_equal, Expression::Operation::kFalse);
  AssertProjectionStrictValue(*spec, bound_less_than, Expression::Operation::kFalse);
  AssertProjectionStrictValue(*spec, bound_less_equal, Expression::Operation::kFalse);
  AssertProjectionStrictValue(*spec, bound_greater_than, Expression::Operation::kFalse);
  AssertProjectionStrictValue(*spec, bound_greater_equal, Expression::Operation::kFalse);
}
...
TEST_F(BucketingProjectionTest, BucketIntegerInclusive) {
  ...
  // The bucket number of 100 with 10 buckets is 6
  AssertProjectionInclusive(*spec, bound_equal, Expression::Operation::kEq, "6");
  AssertProjectionInclusiveValue(*spec, bound_not_equal, Expression::Operation::kTrue);
  AssertProjectionInclusiveValue(*spec, bound_less_than, Expression::Operation::kTrue);
  AssertProjectionInclusiveValue(*spec, bound_less_equal, Expression::Operation::kTrue);
  AssertProjectionInclusiveValue(*spec, bound_greater_than, Expression::Operation::kTrue);
  AssertProjectionInclusiveValue(*spec, bound_greater_equal,
...
TEST_F(BucketingProjectionTest, BucketStringStrict) {
  ...
  AssertProjectionStrict(*spec, bound_not_equal, Expression::Operation::kNotEq, "4");
  AssertProjectionStrictValue(*spec, bound_equal, Expression::Operation::kFalse);
}
...
TEST_F(BucketingProjectionTest, BucketStringInclusive) {
  ...
  // The bucket number of "abcdefg" with 10 buckets is 4
  AssertProjectionInclusive(*spec, bound_equal, Expression::Operation::kEq, "4");
  AssertProjectionInclusiveValue(*spec, bound_not_equal, Expression::Operation::kTrue);
}
...
TEST_F(DateProjectionTest, DayStrict) {
  ...
  AssertProjectionStrict(*spec, bound_less_than, Expression::Operation::kLt);
  AssertProjectionStrict(*spec, bound_less_equal, Expression::Operation::kLt);
  AssertProjectionStrict(*spec, bound_greater_than, Expression::Operation::kGt);
  AssertProjectionStrict(*spec, bound_greater_equal, Expression::Operation::kGt);
  AssertProjectionStrict(*spec, bound_equal, Expression::Operation::kFalse);
}
...
TEST_F(DateProjectionTest, DayInclusive) {
  ...
  AssertProjectionInclusive(*spec, bound_less_than, Expression::Operation::kLtEq);
  AssertProjectionInclusive(*spec, bound_less_equal, Expression::Operation::kLtEq);
  AssertProjectionInclusive(*spec, bound_greater_than, Expression::Operation::kGtEq);
  AssertProjectionInclusive(*spec, bound_greater_equal, Expression::Operation::kGtEq);
  AssertProjectionInclusive(*spec, bound_equal, Expression::Operation::kEq);
}
