...
TEST_F(MergingSnapshotUpdateTest, ApplyCleansMergedDvOnError) {
  SetTableFormatVersion(3);

  ICEBERG_UNWRAP_OR_FAIL(auto first_dv,
                         WriteDeletionVector("/delete/dv_a.puffin", {1}, file_a_));
  ICEBERG_UNWRAP_OR_FAIL(auto second_dv,
                         WriteDeletionVector("/delete/dv_b.puffin", {2}, file_a_));
  ...
  EXPECT_THAT(op->AddDelete(first_dv, 7), IsOk());
  EXPECT_THAT(op->AddDelete(second_dv, 8), IsOk());

  EXPECT_THAT(static_cast<SnapshotUpdate&>(*op).Apply(),
              IsError(ErrorKind::kInvalidArgument));
  EXPECT_THAT(deleted_paths, ::testing::Contains(::testing::HasSubstr("merged-dvs-")));
}
