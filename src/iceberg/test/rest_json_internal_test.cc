...
TEST(RestJsonInternalTest, CreateNamespaceRequest) {
  CreateNamespaceRequest request;
  request.namespaces = {"db1", "schema1"};
  request.properties = std::unordered_map<std::string, std::string>{{"key1", "value1"},
                                                                    {"key2", "value2"}};

  auto json = ToJson(request);
  nlohmann::json expected_json = R"({
    "namespaces": ["db1", "schema1"],
    "properties": {"key1": "value1", "key2": "value2"}
  })"_json;

  EXPECT_EQ(json, expected_json);

  auto parsed_result = CreateNamespaceRequestFromJson(expected_json);
  ASSERT_TRUE(parsed_result.has_value());
  EXPECT_EQ(request.namespaces, parsed_result->namespaces);
  EXPECT_EQ(*request.properties, *parsed_result->properties);
}

TEST(RestJsonInternalTest, CreateNamespaceRequestWithoutProperties) {
  CreateNamespaceRequest request;
  request.namespaces = {"db1"};
  request.properties = std::nullopt;
  ...
  ASSERT_TRUE(parsed_result.has_value());
  EXPECT_EQ(request.namespaces, parsed_result->namespaces);
  EXPECT_FALSE(parsed_result->properties.has_value());
}

TEST(RestJsonInternalTest, CreateNamespaceResponse) {
  CreateNamespaceResponse response;
  response.namespaces = {"db1", "schema1"};
  response.properties = std::unordered_map<std::string, std::string>{{"created", "true"}};
  ...
  auto parsed_result = CreateNamespaceResponseFromJson(expected_json);
  ASSERT_TRUE(parsed_result.has_value());
  EXPECT_EQ(response.namespaces, parsed_result->namespaces);
  EXPECT_EQ(*response.properties, *parsed_result->properties);
}

TEST(RestJsonInternalTest, CreateNamespaceResponseWithoutProperties) {
  CreateNamespaceResponse response;
  response.namespaces = {"db1"};
  response.properties = std::nullopt;
...
TEST(RestJsonInternalTest, GetNamespacePropertiesResponse) {
  GetNamespacePropertiesResponse response;
  response.namespaces = {"db1", "schema1"};
  response.properties = {{"prop1", "value1"}, {"prop2", "value2"}};
  ...
  auto parsed_result = GetNamespacePropertiesResponseFromJson(expected_json);
  ASSERT_TRUE(parsed_result.has_value());
  EXPECT_EQ(response.namespaces, parsed_result->namespaces);
  EXPECT_EQ(response.properties, parsed_result->properties);
}
...
TEST(RestJsonInternalTest, CreateTableRequest) {
  auto schema = std::make_shared<Schema>(
      std::vector<SchemaField>{SchemaField(1, "id", iceberg::int64(), false),
...
TEST(RestJsonInternalTest, LoadTableResponseComplete) {
  std::vector<SchemaField> schema_fields;
  schema_fields.emplace_back(/*field_id=*/1, "x", iceberg::int64(), /*optional=*/false);
  auto schema = std::make_shared<Schema>(std::move(schema_fields), /*schema_id=*/1);

  auto partition_spec =
      std::make_shared<PartitionSpec>(schema, 0, std::vector<PartitionField>{});
  auto sort_order = std::make_shared<SortOrder>(0, std::vector<SortField>{});

  TableMetadata metadata{.format_version = 1,
                         .table_uuid = "test-uuid-load-table",
  ...
  LoadTableResponse response{
      .metadata_location = "/tmp/warehouse/test_table/metadata/v1.metadata.json",
      .metadata = metadata,
  ...
  EXPECT_TRUE(json.contains("metadata-location"));
  EXPECT_TRUE(json.contains("metadata"));
  EXPECT_TRUE(json.contains("config"));

  auto parsed_result = LoadTableResponseFromJson(json);
  ASSERT_TRUE(parsed_result.has_value()) << parsed_result.error().message;

  EXPECT_EQ(*response.metadata_location, *parsed_result->metadata_location);
  EXPECT_EQ(response.metadata.table_uuid, parsed_result->metadata.table_uuid);
  EXPECT_EQ(response.metadata.location, parsed_result->metadata.location);
  EXPECT_EQ(response.metadata.format_version, parsed_result->metadata.format_version);
  EXPECT_EQ(response.metadata.last_column_id, parsed_result->metadata.last_column_id);
  EXPECT_EQ(response.metadata.current_schema_id,
            parsed_result->metadata.current_schema_id);
  EXPECT_EQ(response.metadata.current_snapshot_id,
            parsed_result->metadata.current_snapshot_id);
  EXPECT_EQ(*response.config, *parsed_result->config);
}
